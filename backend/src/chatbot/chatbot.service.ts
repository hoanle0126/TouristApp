...
export class ChatbotService {
  ...
  async respond(input: ChatbotRequest): Promise<ChatbotResponse> {
    ...
    const [tours, hotels, blogs] = bookingResult
      ? [[], [], []]
      : await Promise.all([
    ...
    const groundedResult = bookingResult ?? (shouldGroundResponse
      ? this.buildGroundedResult(message, tours, hotels, blogs)
  ...
  private buildGroundedResult(
    ...
    if (this.isBroadCatalogRequest(message)) {
      ...
      if (availableTour) {
        const openDeparture = availableTour.departures.find(
          (departure) => departure.status === 'open',
        );
        summaryParts.push(
          openDeparture
            ? `${availableTour.title} currently has ${Math.max(openDeparture.capacity - openDeparture.booked, 0)} seats left on ${this.toDateString(openDeparture.date)}.`
            : `${availableTour.title} is listed with ${availableTour.availability} availability on the website.`,
        );
        sources.push({
          kind: 'tour' as const,
      ...
      if (availableHotel) {
        const openInventoryDay = availableHotel.inventoryDays.find(
          (day) => day.status === 'open',
        );
        summaryParts.push(
          openInventoryDay
            ? `${availableHotel.name} currently has ${Math.max(openInventoryDay.totalRooms - openInventoryDay.bookedRooms, 0)} rooms left on ${this.toDateString(openInventoryDay.date)}.`
            : `${availableHotel.name} is listed in ${availableHotel.location} on the website.`,
        );
        sources.push({
          kind: 'hotel' as const,
