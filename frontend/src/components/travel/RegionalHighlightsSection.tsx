...
interface RegionalHighlightsSectionProps {
  readonly ctaHref?: string;
  readonly ctaLabel?: string;
  readonly description?: string;
  readonly eyebrow?: string;
  readonly items: readonly RegionalHighlightItem[];
  readonly reverse?: boolean;
  readonly title?: string;
}

export function RegionalHighlightsSection({
  ctaHref = "/tours",
  ctaLabel = "Explore now",
  description = "Live tour deals from the backend catalog, shown in a horizontal carousel for quick browsing.",
  eyebrow = "Collection",
  items,
  reverse = false,
  ...
  useEffect(() => {
    if (!emblaApi || visibleItems.length <= 1) {
      return;
    }
  ...
  return (
    <section className={`overflow-hidden py-16 text-white md:py-20 ${sectionBackgroundClass}`}>
        ...
        <div className="flex w-full flex-col justify-center lg:min-h-145 lg:w-[320px] lg:flex-none">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <div className="flex gap-2">
              <Button aria-label="Previous item" className="rounded-full border-white/25 bg-white/10 text-white hover:bg-white/20" disabled={!canScrollPrev} onClick={scrollPrev} size="icon" variant="outline">
                <ChevronLeft className="size-5" />
              </Button>
              <Button aria-label="Next item" className="rounded-full border-white/25 bg-white/10 text-white hover:bg-white/20" disabled={!canScrollNext} onClick={scrollNext} size="icon" variant="outline">
        ...
        <div className="min-w-0 flex-1 overflow-hidden py-1" ref={emblaRef}>
          <div className="flex gap-5 pb-3">
            {visibleItems.map((item, index) => (
              <article className="group flex min-h-full min-w-0 flex-[0_0_288px] flex-col overflow-hidden rounded-3xl bg-white text-stone-950 shadow-[0_24px_60px_rgba(15,23,42,0.22)] sm:flex-[0_0_320px] lg:flex-[0_0_328px]" key={`${item.href}-${item.title}`}>
