...
function filterResults(results: readonly SearchResult[], filters: TravelSearchFilters) {
  ...
  const filteredResults = results.filter((result) => {
    ...
    const maxPriceMatch = filters.maxPrice === undefined || filters.maxPrice >= 10000 || price <= filters.maxPrice;
    return categoryMatch && minPriceMatch && maxPriceMatch && matchesDuration(result, filters.duration);
  });

  return sortResults([...filteredResults], filters.sort);
}

export async function searchTravelProducts(query: string, filters: TravelSearchFilters = {}): Promise<SearchResult[]> {
